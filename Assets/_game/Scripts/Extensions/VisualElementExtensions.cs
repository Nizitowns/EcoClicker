...
namespace _game.Scripts.Extensions
{
    public static class VisualElementExtensions {
        ...
        public static T CreateChild<T>(this VisualElement parent, params string[] classes) where T : VisualElement, new() {
            ...
            {
                if (SoundManager.Instance == null) return child;
                button.clickable.clicked += () => SoundManager.Instance.PlayButtonClickSound();
                //child.RegisterCallback<MouseEnterEvent>(evt => { SoundManager.Instance.PlayButtonHoverSound(); });
            }
            return child;
        }
